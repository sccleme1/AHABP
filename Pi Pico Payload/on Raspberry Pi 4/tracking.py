...
while True:
    ...
    try:
        if i >= 180:
            # every 5 seconds, save the images
            cv.imwrite(os.path.join(path, "raw_" + str(picture) + "_" + str(date.strftime('%H:%M:%S')) + ".jpg"), original)
            cv.imwrite(os.path.join(path, "threshold_" + str(picture) + "_" + str(date.strftime('%H:%M:%S')) + ".jpg"), thresh)
            cv.imwrite(os.path.join(path, "target_" + str(picture) + "_" + str(date.strftime('%H:%M:%S')) + ".jpg"), targeted)
